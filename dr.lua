...
local bondPlaceId = 116495829188952

-- auto party join if in bond place
if pid == bondPlaceId then
    ...
    local function findParty()
        if workspace.PartyZones.PartyZone.BillboardGui.StatusLabel.Text == "Waiting for players..." then
            hrp.CFrame = workspace.PartyZones.PartyZone.WorldPivot.CFrame
            return true
        elseif workspace.PartyZones:FindFirstChild("PartyZone1") and workspace.PartyZones.PartyZone1.BillboardGui.StatusLabel.Text == "Waiting for players..." then
            hrp.CFrame = workspace.PartyZones.PartyZone1.WorldPivot.CFrame
            return true
        elseif workspace.PartyZones:FindFirstChild("PartyZone2") and workspace.PartyZones.PartyZone2.BillboardGui.StatusLabel.Text == "Waiting for players..." then
            hrp.CFrame = workspace.PartyZones.PartyZone2.WorldPivot.CFrame
            return true
        elseif workspace.PartyZones:FindFirstChild("PartyZone3") and workspace.PartyZones.PartyZone3.BillboardGui.StatusLabel.Text == "Waiting for players..." then
            hrp.CFrame = workspace.PartyZones.PartyZone3.WorldPivot.CFrame
            return true
        end
        return false
    end

    while not findParty() do
        task.wait(0.5)
    end
    ...
    gui.Name = "Light Hub | Auto Bond | Menu"
    ...
    title.Text = "Dead Rails V2 | Light Hub"
    ...
    if not game.CoreGui:FindFirstChild("BondFarm") then
        ...
        title2.Text = "Auto-Farm Bond | Light Hub"
    ...
    task.spawn(function()
        while true do
            ...
            if collectedBonds >= 999 then
                ...
                TeleportService:Teleport(bondPlaceId, player)
                break
            elseif collectedBonds >= 2000 then
                task.wait(15)
                queue_on_teleport([[
                    loadstring(game:HttpGet("https://raw.githubusercontent.com/Filipp947/ccp/refs/heads/main/dr.lua"))()
                ]])
...
-- queue on teleport when making party & detect death in loop
while wait(0.1) do
    ...
    -- also queue on teleport when making a party
    local args = {{
        isPrivate = true,
        maxMembers = 1,
        trainId = "default",
